...
class SimpleSearch extends Component<{}, SearchState> {
  constructor(props: {}) {
    super(props);
    this.state = {
      searchId: '',
  ...
  handleSearch = async () => {
    ...
    try {
      ...
      const allCustomersSnap = await getDocs(nameQuery);
...
data };
      ...
      if (foundCustomer) {
        ...
        Alert.alert('Not Found', 'No approved customer found with this name or NIC');
      }
    } catch (error) {
  ...
  processCustomerResult = async (id: string, customerData: any, accountsRef: any, searchNic: string) => {
    ...
    this.setState({
      searchResult: {
        ...
        status: customerData.status || 'approve',
        currentDay,
        ...
        <ScrollView>
          <View className="p-5">
            ...
            <View className="bg-white rounded-lg p-5 mb-5 shadow-md">
              <Text className="text-lg font-bold mb-4 text-gray-800">Approved Customer Search</Text>
                  ...
                  <View className="space-y-3 mb-2">
                    {Object.entries(searchResult.weeklyPayments).map(([day, status]) => {
                      ...
                      return (
                        <View
                          key={day}
                          className={`flex-row items-center justify-between p czterech rounded-lg border-2 mb-5 relative ${
                  ...
                  {searchResult.unpaidDays.length > 5 && (
                    ...
                    >
                      <Ionicons
                        ...
                        color="#fff"
                      />
                      <Text className="text-white among-bold ml-2">
